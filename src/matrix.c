...
const int MATRIX_MAX_ITER =
    1000; ///< Maximum number of iterations for any numerical method on Matrices
const double MATRIX_TOLERANCE =
    1e-6; ///< Maximum margin of error for floating point arithmetic on Matrices
...
/**
 * @brief Draw a random value from the Standard Normal Distribution using the
 ...
 * @return true if the matrices have the same dimension, and the entries are
 * within the tolerance of each other
 * @return false otherwise
 */
bool Matrix_approx_equal(const Matrix *mat1, const Matrix *mat2,
                         double tolerance) {
    if (mat1->rows != mat2->rows || mat1->cols != mat2->cols) {
        return false;
    }
...
double Matrix_frobenius_norm(const Matrix *mat) {
    ...
    return sqrt(Matrix_trace(&mat_matT));
}
 ...
 * @throw Exception for Matrices that are not row vectors (Must have dimension
 * (Nx1))
 */
 ...
 * @throw Exception requires 0 <= start_row < end_row <= mat->rows && 0 <=
 * start_col < end_col <= mat->cols
 */
Matrix Matrix_submatrix(const Matrix *mat, int start_row, int end_row,
                        int start_col, int end_col) {
    if (start_row < 0 || start_row >= mat->rows || end_row <= start_row ||
        end_row > mat->rows || start_col < 0 || start_col >= mat->cols ||
        end_col <= start_col || end_col > mat->cols) {
 ...
 *
 * @param mat Matrix
 * @return SVDResult containing orthonormal Matrices U, V, and diagonal Matrix
 * Sigma with entries in decreasing order
 */
