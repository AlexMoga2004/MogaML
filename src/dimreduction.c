...
/**
 * @brief Computes a low-rank approximation of a matrix using Principle
 * Component Analysis
 *
 * @param X the Matrix
...
Matrix PCA_Reduce(const Matrix *X, unsigned int target_rank) {
    ...
    // Reduce Sigma matrix to target_rank by setting singular values beyond
    // target_rank to zero
    ...
    Matrix Sigma_reduced =
        Matrix_submatrix(&svd.Sigma, 0, target_rank, 0, target_rank);
    Matrix VT_reduced = Matrix_submatrix(&svd.V, 0, target_rank, 0, svd.V.cols);

    // Compute the reduced matrix: result = U_reduced * Sigma_reduced *
    // VT_reduced
...
static double compute_chi_squared(const Matrix *X, const Matrix *y,
                                  int feature_index) {
    ...
    for (int i = 0; i < num_classes; ++i) {
        expected[i] =
            (observed[0] + observed[1]) * (bin_totals[i] / num_samples);
    }
...
static void sort_indices_by_scores(double *scores, int *indices,
                                   int num_features) {
...
/**
 * @brief Selects the most relevant input features using the chi-squared
 * statistical test
 *
 ...
 */
Matrix ChiSquared_Reduce(const Matrix *X, const Matrix *y,
                         unsigned int target_features) {
    int num_features = X->cols;

    if (target_features > num_features) {
        fprintf(stderr,
                "Error in ChiSquared_Reduce: target_features exceeds the "
