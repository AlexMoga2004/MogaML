...
Layer NN_create_layer(int num_neurons, int input_size,
...
NeuralNetwork NN_create_network(int *layer_sizes, int num_layers,
                                ActivationFunction activation) {
    ...
    for (int i = 0; i < num_layers; ++i) {
        network.layers[i] = NN_create_layer(
            layer_sizes[i], i == 0 ? 0 : layer_sizes[i - 1], activation);
    }
...
Matrix NN_forward_pass(NeuralNetwork *network, const Matrix *input) {
    ...
    for (int layer_num = 0; layer_num < network->num_layers; ++layer_num) {
        ...
        for (int neuron_num = 0; neuron_num < current_layer.num_neurons;
             ++neuron_num) {
            ...
            for (int i = 0; i < neuron.weights.rows; ++i) {
                preactivation +=
                    neuron.weights.data[i][0] * previous_output.data[i][0];
            }
            activation.data[neuron_num][0] =
                neuron.activation.apply(preactivation);
        }
...
void NN_backward_pass(NeuralNetwork *network, const Matrix *input,
    ...
    if (activations == NULL || z_values == NULL) {
        fprintf(stderr, "Error in NN_backward_pass, failed to initialise "
                        "activations and/or z_values");
        exit(EXIT_FAILURE);
    }
    ...
    for (int layer_num = 0; layer_num < num_layers; ++layer_num) {
        ...
        for (int neuron_num = 0; neuron_num < current_layer.num_neurons;
            ...
            for (int i = 0; i < neuron.weights.rows; ++i) {
                preactivation += neuron.weights.data[i][0] *
                                 activations[layer_num].data[i][0];
            }
    ...
    for (int layer_num = num_layers - 1; layer_num >= 0; --layer_num) {
        ...
        for (int neuron_num = 0; neuron_num < current_layer.num_neurons;
            ...
            double delta = output_error.data[neuron_num][0] *
                           neuron->activation.apply_derivative(z);

            layer_error.data[neuron_num][0] = delta;

            for (int i = 0; i < neuron->weights.rows; ++i) {
                neuron->weights.data[i][0] -=
                    0.01 * delta * activations[layer_num].data[i][0];
            }
        ...
        if (layer_num > 0) {
            ...
            for (int i = 0; i < activations[layer_num].rows; ++i) {
                ...
                for (int neuron_num = 0; neuron_num < current_layer.num_neurons;
                     ++neuron_num) {
                    Neuron neuron = current_layer.neurons[neuron_num];
                    sum += neuron.weights.data[i][0] *
                           layer_error.data[neuron_num][0];
                }
